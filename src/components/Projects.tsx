...
import { Calendar, MessageSquare, FileText, ArrowUpRight, Github } from 'lucide-react';
...
const projects = [
  {
    ...
    icon: Calendar,
    tags: ['React', 'Framer Motion', 'Google Calendar API', 'OpenAI', 'Node.js'],
  ...
  },
  {
    ...
    icon: MessageSquare,
    tags: ['Next.js 14', 'TypeScript', 'Streaming', 'Edge Runtime', 'Vercel AI SDK'],
  ...
  },
  {
    ...
    span: 'md:col-span-1',
    metrics: [
      { label: 'Full-text search latency', value: '<50ms' },
    ],
  },
];
    ...
    <section id="projects" className="py-32 px-6">
      <div className="max-w-6xl mx-auto">
        <motion.div
          ...
          className="mb-16"
        >
          <p className="mono text-xs text-sky-400 tracking-widest mb-3 uppercase">Projects</p>
          ...
          {projects.map((project, i) => {
            ...
            return (
              <motion.div
                ...
                animate={inView ? { opacity: 1, y: 0 } : {}}
                transition={{ duration: 0.6, delay: 0.1 + i * 0.13, ease: [0.16, 1, 0.3, 1] }}
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-5">
                    <div className={`p-2.5 rounded-xl ${project.iconBg} border ${project.border}`}>
                      <Icon size={18} className={project.iconColor} />
                    </div>
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      <button className="p-1.5 rounded-lg border border-white/10 text-neutral-500 hover:text-white transition-colors">
                        <Github size={14} />
                      </button>
                      <button className="p-1.5 rounded-lg border border-white/10 text-neutral-500 hover:text-white transition-colors">
                  ...
                  <p className="mono text-xs text-neutral-600 mb-1">{project.subtitle}</p>
                  ...
                  {project.metrics.length > 0 && (
                    <div className="flex gap-4 mb-5 pb-5 border-b border-white/[0.05]">
                      {project.metrics.map((m) => (
                        <div key={m.label}>
                          <p className={`text-xl font-bold ${project.iconColor} tabular-nums`}>{m.value}</p>
